...
export default function TimetablePage() {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    setError("");
    setMessage("");
    const url = editingId ? `/api/shedule/${editingId}` : "/api/shedule";
    ...
    setMessage(editingId ? "Class updated." : "Class added to shedule.");
    setForm(emptyForm);
    setEditingId(null);
    load();
  }
  ...
  async function handleDelete(id: string) {
    if (!confirm("Remove this class from the shedule?")) return;
  ...
  return (
    <div>
      <div className="card">
        <h3 style={{ marginTop: 0 }}>{editingId ? "Edit Class" : "Add Class to shedule"}</h3>
      ...
      {loading ? (
        <div className="card">Loading shedule...</div>
      ) : (
